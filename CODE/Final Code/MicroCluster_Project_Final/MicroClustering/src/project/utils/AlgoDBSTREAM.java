...
public List<Cluster> runAlgorithm(String inputFile, int minPts, double epsilon, String separator) throws NumberFormatException, IOException {
		...
		System.out.println("All Data Points: "+points);
		// close the file
		reader.close();
		...
		System.out.println("KD Tree: "+kdtree.toString());
		...
		for(DoubleArray point : points) {
			...
			if(pointDBS.visited == false) {
				...
				System.out.println("neighbours: "+kdtree.pointsWithinRadiusOf(pointDBS, epsilon, bufferNeighboors1));
		...
		File file = new File("D:/MicroCluster/Files/"+"Noise"+".csv");
		csvWriter = new CSVWriter(new FileWriter(file));
		// it is noise
		for(DoubleArray point: points) {
			if(((DoubleArrayDBS)point).cluster == null){
				System.out.println("Noise Points: "+point);
				String[] values;
				values=(Double.toString(point.get(0))+","+Double.toString(point.get(1))).split(",");
	...
	public void saveToFile(String output) throws IOException {
		...
		for(int i=0; i< clusters.size(); i++){
			...
			if(clusters.get(i).getVectors().size() >= 1){
				...
				System.out.println("Clusters:"+clusters.get(i).toString());
				File file = new File("D:/MicroCluster/Files/"+"cluster"+i+".csv");
				CSVWriter csvWriter = new CSVWriter(new FileWriter(file));
				List<DoubleArray> list = clusters.get(i).getVectors();
				String[] values;
				for(int j=0;j<list.size();j++){
					values=(Double.toString(list.get(j).get(0))+","+Double.toString(list.get(j).get(1))).split(",");
					csvWriter.writeNext(values);
				}
				csvWriter.close();
	...
	public void printStatistics() {
		System.out.println("========== DBSTREAM STATS ============");
